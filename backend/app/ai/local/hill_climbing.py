...
def _neighbours(plan: Plan):
    for idx, session in enumerate(plan.sessions):
        ...
        for day in range(DAYS_IN_WEEK):
            if day == session.day:
                continue
            yield _move_session(plan, idx, day)


def _move_session(plan: Plan, idx: int, new_day: int) -> Plan:
    neighbour = plan.model_copy(deep=True)
    original: ScheduledSession = neighbour.sessions[idx]
    neighbour.sessions[idx] = original.model_copy(update={"day": new_day})
    return neighbour
